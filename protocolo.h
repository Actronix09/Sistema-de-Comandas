...
typedef enum {
    ...
    OP_LISTAR_TICKETS = 10,          // NUEVO
    OP_MARCAR_TICKET_PAGADO = 11     // NUEVO
} TipoOperacion;
...
// Agregar nueva estructura para tickets en Respuesta
typedef struct {
    int id;
    int pedido_id;
    char mesa[50];
    ...
    int num_items;
    float total;
    int estado; // 0=Pendiente, 1=Pagado
    char fecha[20];
} TicketMsg;
...
typedef struct {
    ...
    int pedido_id;
    int nuevo_estado;
    float total_pedido; //NUEVO PARA TICKET
} Peticion;
...
typedef struct {
    ...
    // NUEVO: Para tickets
    TicketMsg tickets[50];
    int num_tickets;
    int ticket_id_pagado;
} Respuesta;
